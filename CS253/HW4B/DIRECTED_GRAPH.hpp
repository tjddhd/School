 ...
 * \authors Lisa Tatro & Thomas Dolan
...
{	
	...
	if (N>0)
	{
		...
		for(unsigned int i = 0; i < COLLECTION<T>:: m_size; i++)
		{
			try
			{
		...
		for( unsigned int i = 0; i < COLLECTION<T>:: m_size; i++)
		{
			for( unsigned int j =0; j < COLLECTION<T>::m_size; j++)
			{
				m_Edges_Array[i][j] = 0;
			}
		}
...
bool DIRECTED_GRAPH<T,N>::isSimple() const
{
	...
	for(unsigned int i = 0; i< COLLECTION<T>::m_size; i++)
	{
		for(unsigned int j = i+1; j <COLLECTION<T>::m_size; j++)
		{
...
//Prim's algorithm
template<class T, unsigned int N>
void prim(DIRECTED_GRAPH<T, N> & G, int size, int visitedFrom[])
{
	//Invariant: the subgraph is the minimal distance need to span those points
	int selected[747]/*,visitedFrom[747]*/,min,i,j,y=0,edgeNo,total=0;
	bool isVisited[747];

	for(int i = 0; i < size; i++)
	{
		selected[i] = NOT_YET_VISITED;
		visitedFrom[i] = NOT_YET_VISITED;
		isVisited[i] = false;
	}

	selected[121] = 0;
	edgeNo = 0;
	visitedFrom[121] = 121; //Corresponding index to Washington DC
	isVisited[121] = true;
	...
	while(edgeNo < size)
	{
		...
		for(i=0;i<size;i++)
		{
			if(selected[i]==0)
			{
				for(j=0;j<size;j++)
				{
					if(selected[j]==NOT_YET_VISITED && !(isVisited[j]))
					{
						if(min>G.getEdgeWeight(i,j))
						{
							min=G.getEdgeWeight(i,j);
							y=j;
							isVisited[j]=true;
							visitedFrom[j] = i;
						}
...
template<class T, unsigned int N>
void dijkstra(DIRECTED_GRAPH<T, N> & G, int size)
{
	//Invariant: the subgraph is the minimal distance need to span those points
	int selected[747],visitedFrom[747],min,i,j,y=0,edgeNo,total=0;
	bool isVisited[747];
...
//Relax relation
template<class T, unsigned int N>
void relax(int u, int v, int visitedFrom[], DIRECTED_GRAPH<T, N> & G)
{
	if(G.getEdgeWeight(121,v)>G.getEdgeWeight(121,u)+G.getEdgeWeight(u,v))
	{
		G.setEdgeWeight(121,v,G.getEdgeWeight(121,u)+G.getEdgeWeight(u,v));
		visitedFrom[v] = u;
	}
}

//Edmonds-Karp alogrithm
template<class T, unsigned int N>
void edmondsKarp(DIRECTED_GRAPH<T, N> & G)
{
	//Invariant: the subgraph is the minimal distance need to span those points
	while(G.isComplete())
	{
