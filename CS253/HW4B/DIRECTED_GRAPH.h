...
public:
  ...
  //Purpose: construct the shortest spanning path; used in Edmonds-Karp
  //Parameters: graph and its size
  //Post: A minimal spanning tree
  void dijkstra(DIRECTED_GRAPH<T, N> & G, int size);

  //Purpose: To find whether we can improve the shortest path; used in Dijkstra
  //Parameters: node indices u and v, the visitedFrom array, and the graph
  //Post: visitedFrom will be updated if needed
  void relax(int u, int v, int visitedFrom[], DIRECTED_GRAPH<T, N> & G);

  //Purpose: construct a flow network for optimality
  //Parameters: graph
  //Post: An optimal flow network
