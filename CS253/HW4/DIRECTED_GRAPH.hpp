...
void prim(DIRECTED_GRAPH<T, N> & G, int size)
{
	...
	int v=0, u, m, k=1, top =0;
	int visit[757], visited[757], order[757];
	while(k<size)
	{
		m = 31999;
		if(k==1)
		{
			for(int i=1; i <= size; i++)
			{
				...
				{
					if(G.getEdgeWeight(i,j)<m)
					{
						m = G.getEdgeWeight(i,j);
						u =i;
					}
				}
			}
		...
		else
		{
			...
			{
				if(G.getEdgeWeight(v,j)<m && visited[j]!=1 && visit[j]!=1)
				{
					visit[j] = 1;
					order[top]=j;
					top++;
	...
	for(int i =0; i < 756; i++)
	{
		temp += G.getEdgeWeight(order[i], order[i+1]);
	}
	cout<<"MST Weight: "<<temp<<endl;
}
