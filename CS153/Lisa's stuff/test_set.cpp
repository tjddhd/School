...
TEST(SetTest, FindDoesNotExist)
{
   ...
   */
   ...
   {
     a.insert(i);
     ASSERT_EQ(false, a.find((a.max_size()+1)));
	}
}
...
TEST(SetTest, UnionWithDuplicates)
{
   ...
   * the two sets being unioned have duplicate items
   */
   Set <int> a,b;
  for (int i=0; i<a.max_size()/2; i++)
  {
